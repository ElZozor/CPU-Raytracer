...
Scene *initScene3()
{
  ...
  {
    SceneParameters::focalDistance = 2.f;
  }
...
Scene *initScene4()
{
  ...
  if (SceneParameters::focalDistance == -1.f)
  {
    SceneParameters::focalDistance = 0.1f;
    SceneParameters::focalRange = 3.f;
  }
...
inline const std::pair<std::string, std::string> separateargs(const std::string& arg)
{
  size_t pos = arg.find('=');
  ...
  return std::pair<std::string, std::string> (
    arg.substr(0, pos), arg.substr(pos + 1)
  );
}
...
{
  if (argc < 4) return;
  const std::set<std::string> validAAMode({"none", "luma", "sobel"});
  std::vector<std::string> args(argv + 3, argv + argc);
  for (const auto& arg: args)
  {
    auto [name, value] = separateargs(arg);
    ...
    {
      iss >> SceneParameters::imageWidth;
    }
    else if (name == "height")
    {
      iss >> SceneParameters::imageHeight;
    }
    else if (name == "aa")
    {
      iss >> SceneParameters::antiAliasing;
    }
    else if (name == "aamode")
    {
      if (validAAMode.find(value) != validAAMode.end())
      {
        iss >> SceneParameters::AAMode;
      }
      else
      {
    ...
    else if (name == "focaldist")
    {
      iss >> SceneParameters::focalDistance;
    }
    else if (name == "focalrange")
    {
      iss >> SceneParameters::focalRange;
    }
    else
    {
...
int main(int argc, char *argv[])
{
  ...
  int scene_id = atoi(argv[2]);;

  SceneParameters::imageWidth  = WIDTH;
  SceneParameters::imageHeight = HEIGHT;

  parseArguments(argc, argv);

  Image *img = initImage(SceneParameters::imageWidth, SceneParameters::imageHeight);
  Scene *scene = NULL;
  switch (scene_id)
  {
  case 0:
