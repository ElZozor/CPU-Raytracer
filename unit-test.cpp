...
int main(void){
  ...
  for(int i=0; i<fresnelExpectedCount; i++){
    float fresnelvalue = abs(fresnelExpected[i].res - RDM_Fresnel(fresnelExpected[i].LdotH, fresnelExpected[i].extIOR,fresnelExpected[i].intIOR));
    fresnel &= fresnelvalue<0.0001f;
  }
